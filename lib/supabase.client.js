...
export async function signInWithPassword(email, password) {
  ...
  if (error) {
    // Normalize the most common Supabase errors into something user-friendly.
    const msg = (error.message || '').toLowerCase();
    if (msg.includes('invalid') || msg.includes('credentials')) {
      return { ok: false, error: 'Incorrect email or password.' };
    }
    if (msg.includes('not confirmed')) {
      return { ok: false, error: 'Email not confirmed. Check your inbox for the verification link.' };
    }
...
export async function signInWithMagicLink(email) {
  ...
  const { error } = await c.auth.signInWithOtp({
    ...
    options: {
      ...
      // Magic-link is a fallback now (e.g. forgot password). Don't auto-create
      // a user since you'll already have one set up via the dashboard.
