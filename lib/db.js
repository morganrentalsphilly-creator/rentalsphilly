...
// Pull the current Supabase access token (if signed in) so we can attach
// `Authorization: Bearer <token>` to /api/data calls. Server verifies the
// token + admin allowlist before doing any work. Calls from the unauthenticated
// intake form (action: create_lead) simply send no header — the server handles
