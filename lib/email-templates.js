...
export function welcomeEmailHtml({ firstName, emailBody, emailSubject, agentName }) {
  ...
  return htmlShell({
    ...
    footerNote: agentName ? `${escapeHtml(agentName)} · Skale Real Estate` : '',
  });
}
...
export function curatedLinkHtml({ firstName, agentName, url }) {
  return htmlShell({
    ...
    body: `
      <p style="margin:0 0 14px;font-size:17px;color:${BRAND_INK};font-weight:600;">Hi ${escapeHtml(firstName)},</p>
      <p style="margin:0 0 18px;">I hand-picked rentals on BrightMLS that match what you're looking for. Take a look and let me know which ones interest you:</p>
