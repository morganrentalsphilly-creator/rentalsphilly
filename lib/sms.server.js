...
function appendCompliance(body, kind) {
  ...
  const transactional = new Set([
    ...
    'tour_debrief',                 // direct follow-up to a tour the lead booked
...
export async function sendSms({
  ...
  //
  // CRITICAL: messages.id is NOT NULL with no DB default (same class of bug
  // we fixed for the inbound webhook in #181). Without an explicit id this
  // insert silently failed — every blast / nudge against an opted-out lead
  // would have appeared to succeed but left no row, and crucially we'd lose
  // the `idempotency_key` row so the next retry would attempt to send again.
  if (lead.opted_out) {
    const optOutRowId = `m_${Date.now()}_${Math.random().toString(36).slice(2, 6)}`;
    const { data: row, error: optOutInsertErr } = await db
      .from('messages')
      .insert({
        id: optOutRowId,
        lead_id: lead.id,
        ...
        // ALWAYS persist an idempotency_key here. If the caller didn't pass
        // one, we generate one — otherwise a retry would re-enter this path,
        // re-insert another opt-out row, and the count of "blocked sends"
        // would inflate. The UNIQUE constraint on idempotency_key prevents
        // that.
      ...
      .single();
    if (optOutInsertErr) {
      console.error('[SMS — OPT-OUT ROW INSERT FAILED]', {
        leadId: lead.id, kind, error: optOutInsertErr.message, code: optOutInsertErr.code,
      });
    }
    if (blastRecipientId) {
      await db.from('sms_blast_recipients')
        .update({ status: 'opted_out', message_id: row?.id || optOutRowId })
